...
func _physics_process(delta: float) -> void:
	...
	if ping_level > 0 and ping_timer <= 0.0:
		...
		for pulse_index in range(ping_level):
			var pulse_ratio := float(pulse_index + 1) / float(ping_level)
			var pulse_radius := ping_radius * lerpf(0.55, 1.0, pulse_ratio)
			var damage_scale := 1.0 if pulse_index == ping_level - 1 else 0.28 + float(ping_level) * 0.035
		...
		if projection != null and projection.call("is_targetable") and projection.global_position.distance_to(player.global_position) <= ping_radius:
			projection.call("take_shell_damage", pulse_damage * float(ping_level) * 0.8)
	...
	if firewall_level > 0 and firewall_timer <= 0.0:
		...
		for layer in range(1, firewall_level + 1):
			var layer_radius := _firewall_radius(layer)
			_add_ring(player.global_position, layer_radius, Color("efb23d"), 0.18 + float(layer) * 0.025)
		swarm.call("push_area", player.global_position, outer_radius, 10.0 + float(firewall_level) * 4.0)
...
func apply_upgrade(upgrade_id: String) -> void:
	match upgrade_id:
		"bash": bash_level = mini(5, bash_level + 1)
		"ping": ping_level = mini(5, ping_level + 1)
		"firewall": firewall_level = mini(5, firewall_level + 1)
		"log": log_level = mini(5, log_level + 1)
		"wrench": wrench_level = mini(5, wrench_level + 1)
		"rule_chain": rule_chain_level = mini(5, rule_chain_level + 1)
		"lock_zone": lock_zone_level = mini(5, lock_zone_level + 1)
		"worker": worker_level = mini(5, worker_level + 1)
...
func get_upgrade_card(upgrade_id: String) -> Dictionary:
	...
	match upgrade_id:
		"bash":
			return {
				"id": upgrade_id,
				"name": "Bash 脚本  STACK %d → %d" % [current, next],
				"title": "Bash 脚本叠加至 %d 层" % next,
		...
		"ping":
			var ping_description := "脉冲数 %d→%d · 半径 %d→%dpx · 单次伤害 %d→%d · 冷却 %.2f→%.2fs" % [
				current, next, int(_ping_radius(current)), int(_ping_radius(next)), int(_ping_damage(current)), int(_ping_damage(next)),
			...
			return {
				"id": upgrade_id,
				"name": "Ping 扫描  STACK %d → %d" % [current, next],
				"title": "Ping 扫描叠加至 %d 层" % next,
				"description": ping_description,
				"color": Color("47c9f1"),
			}
		"firewall":
			var firewall_description := "规则环 %d→%d · 外圈 %d→%dpx · 每跳伤害 %.0f→%.0f · 击退同步增强" % [
				current, next, int(_firewall_radius(current)), int(_firewall_radius(next)), _firewall_damage(current), _firewall_damage(next),
			]
			if current == 0:
				firewall_description = "解锁持续规则环 · 半径 %dpx · 每跳伤害 %.0f · 自动击退近身故障" % [int(_firewall_radius(next)), _firewall_damage(next)]
			return {
				"id": upgrade_id,
				"name": "防火墙规则  STACK %d → %d" % [current, next],
				"title": "防火墙叠加至 %d 层" % next,
		...
		"log":
			...
			return {
				"id": upgrade_id,
				"name": "日志采集器  STACK %d → %d" % [current, next],
				"title": "日志采集叠加至 %d 层" % next,
		...
		"wrench":
			...
			return {
				"id": upgrade_id,
				"name": "机柜扳手  STACK %d → %d" % [current, next],
				"title": "机柜扳手叠加至 %d 层" % next,
				"description": wrench_description,
				"color": Color("ffb454"),
			}
		"rule_chain":
			var chain_description := "规则节点 %d→%d · 轨道 %d→%dpx · 接触伤害 %.0f→%.0f · 转速同步提升" % [
			...
			if current == 0:
				chain_description = "解锁实体环绕规则链 · %d 个 ACL 节点持续切割近身故障" % _rule_chain_nodes(next)
			return {
				"id": upgrade_id,
				"name": "iptables 规则链  STACK %d → %d" % [current, next],
				"title": "规则链叠加至 %d 层" % next,
		...
		"lock_zone":
			...
			return {
				"id": upgrade_id,
				"name": "慢查询锁域  STACK %d → %d" % [current, next],
				"title": "慢查询锁域叠加至 %d 层" % next,
		...
		"worker":
			...
			return {
				"id": upgrade_id,
				"name": "Worker Pod  STACK %d → %d" % [current, next],
				"title": "Worker Pod 叠加至 %d 层" % next,
		...
		"arch_zero_trust":
			return {
				...
				"description": "签名技能：iptables 规则链 · 每阶 +1 环绕节点并提高接触伤害",
				"color": Color("ee6677"),
			}
		"arch_query":
			return {
...
func play_upgrade_burst(upgrade_id: String, accent: Color = Color(0.0, 0.0, 0.0, 0.0), stack_override: int = 0) -> void:
	...
	var ring_count := 5 if upgrade_id == "iac" else mini(5, 2 + stack_count)
	for ring_index in range(ring_count):
		_add_ring(player.global_position, 46.0 + float(ring_index) * 24.0, color, 0.38 + float(ring_index) * 0.08)
...
func _add_ring(center: Vector2, radius: float, color: Color, lifetime: float) -> void:
	_append_effect({"type": "ring", "center": center, "radius": radius, "color": color, "ttl": lifetime, "max": lifetime})
...
func _draw() -> void:
	if player != null and firewall_level > 0:
		...
		for layer in range(1, firewall_level + 1):
			var radius := _firewall_radius(layer)
	...
	for effect in effects:
		...
		match String(effect["type"]):
			...
			_:
				draw_arc(Vector2(effect["center"]), float(effect["radius"]) * (1.1 - alpha * 0.1), 0.0, TAU, 48, color, 3.0)


func _bash_damage(level: int) -> float:
	return 0.0 if level <= 0 else (16.0 + float(level) * 4.0) * career_damage_multiplier * temporary_damage_multiplier
...
func _bash_cooldown(level: int) -> float:
	if level <= 0:
		return 0.0
	var value := (0.58 - float(level) * 0.032) * (1.0 - float(idempotency_level) * 0.04)
	if evolved_iac:
		value *= 0.88
	return maxf(0.16, value * career_cooldown_multiplier)


func _bash_range(level: int) -> float:
	return 0.0 if level <= 0 else (370.0 + float(level) * 20.0 + (60.0 if evolved_iac else 0.0)) * career_area_multiplier


func _bash_targets(level: int) -> int:
	return 0 if level <= 0 else 1 + floori(float(level) / 2.0)
...
func _ping_radius(level: int) -> float:
	return 0.0 if level <= 0 else (120.0 + float(level) * 24.0) * career_area_multiplier


func _ping_damage(level: int) -> float:
	return 0.0 if level <= 0 else (15.0 + float(level) * 3.0) * career_damage_multiplier * temporary_damage_multiplier


func _ping_cooldown(level: int) -> float:
	return 0.0 if level <= 0 else maxf(1.8, (4.45 - float(level) * 0.45) * career_cooldown_multiplier)


func _firewall_radius(level: int) -> float:
	return 0.0 if level <= 0 else (60.0 + float(level) * 10.0) * career_area_multiplier


func _firewall_damage(level: int) -> float:
	return 0.0 if level <= 0 else (6.75 + float(level) * 2.25) * career_damage_multiplier * temporary_damage_multiplier


func _firewall_cooldown(level: int) -> float:
	return 0.0 if level <= 0 else maxf(0.38, (0.78 - float(level) * 0.06) * career_cooldown_multiplier)
...
func _log_radius(level: int) -> float:
	return 0.0 if level <= 0 else (51.0 + float(level) * 9.0) * career_area_multiplier


func _log_damage(level: int) -> float:
	return 0.0 if level <= 0 else (22.0 + float(level) * 4.0) * career_damage_multiplier * temporary_damage_multiplier


func _log_cooldown(level: int) -> float:
	return 0.0 if level <= 0 else maxf(1.8, (3.45 - float(level) * 0.25) * career_cooldown_multiplier)


func _log_secondary_ratio(level: int) -> float:
	return 0.30 + float(level) * 0.03


func _wrench_sweeps(level: int) -> int:
	if level <= 0:
		return 0
	return 1 + floori(float(level - 1) / 2.0) + (1 if oncall_arch_level >= 2 else 0)


func _wrench_reach(level: int) -> float:
	return 0.0 if level <= 0 else (78.0 + float(level) * 10.0) * career_area_multiplier


func _wrench_half_angle(level: int) -> float:
	return 0.0 if level <= 0 else 0.68 + float(level) * 0.035


func _wrench_damage(level: int) -> float:
	if level <= 0:
		return 0.0
	return (27.0 + float(level) * 7.0) * (1.0 + float(oncall_arch_level) * 0.18) * career_damage_multiplier * temporary_damage_multiplier * career_melee_multiplier


func _wrench_cooldown(level: int) -> float:
	return 0.0 if level <= 0 else maxf(0.36, (0.94 - float(level) * 0.075) * career_cooldown_multiplier)


func _rule_chain_nodes(level: int) -> int:
	if level <= 0:
		return 0
	return 1 + ceili(float(level) / 2.0) + zero_trust_arch_level


func _rule_chain_radius(level: int) -> float:
	return 0.0 if level <= 0 else (68.0 + float(level) * 8.0) * career_area_multiplier


func _rule_chain_hit_radius(level: int) -> float:
	return 0.0 if level <= 0 else 13.0 + float(level) * 1.5


func _rule_chain_damage(level: int) -> float:
	if level <= 0:
		return 0.0
	return (4.2 + float(level) * 1.8) * (1.0 + float(zero_trust_arch_level) * 0.12) * career_damage_multiplier * temporary_damage_multiplier


func _rule_chain_tick(level: int) -> float:
	return 0.0 if level <= 0 else maxf(0.16, (0.36 - float(level) * 0.025) * career_cooldown_multiplier)


func _rule_chain_speed(level: int) -> float:
	return 0.0 if level <= 0 else 1.10 + float(level) * 0.14
...
func _lock_zone_limit(level: int) -> int:
	return 0 if level <= 0 else 1 + floori(float(level - 1) / 2.0)


func _lock_zone_radius(level: int) -> float:
	return 0.0 if level <= 0 else (60.0 + float(level) * 9.0 + float(query_arch_level) * 10.0) * career_area_multiplier


func _lock_zone_duration(level: int) -> float:
	return 0.0 if level <= 0 else (2.45 + float(level) * 0.38 + float(query_arch_level) * 0.70) * career_control_multiplier


func _lock_zone_damage(level: int) -> float:
	return 0.0 if level <= 0 else (5.5 + float(level) * 2.5) * career_damage_multiplier * temporary_damage_multiplier


func _lock_zone_tick(level: int) -> float:
	return 0.0 if level <= 0 else maxf(0.40, 0.62 - float(level) * 0.025)


func _lock_zone_cooldown(level: int) -> float:
	return 0.0 if level <= 0 else maxf(2.15, (5.10 - float(level) * 0.45) * career_cooldown_multiplier)


func _worker_count(level: int) -> int:
	if level <= 0:
		return 0
	return 1 + floori(float(level - 1) / 2.0) + autoscale_arch_level + career_summon_bonus


func _worker_targets_per_unit(level: int) -> int:
	return 0 if level <= 0 else 1 + (1 if level >= 4 else 0)


func _worker_range(level: int) -> float:
	return 0.0 if level <= 0 else (280.0 + float(level) * 24.0) * career_area_multiplier


func _worker_damage(level: int) -> float:
	if level <= 0:
		return 0.0
	return (8.0 + float(level) * 3.5) * (1.0 + float(autoscale_arch_level) * 0.12) * career_damage_multiplier * temporary_damage_multiplier * career_summon_damage_multiplier


func _worker_cooldown(level: int) -> float:
	return 0.0 if level <= 0 else maxf(0.50, (1.05 - float(level) * 0.07) * career_cooldown_multiplier)
...
func _worker_positions() -> Array[Vector2]:
	...
	var formation_radius := 112.0 + float(worker_level) * 3.0
